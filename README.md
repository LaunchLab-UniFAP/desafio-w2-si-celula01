...
💬 RESPOSTA DA CÉLULA:A arquitetura foi feita separando os dados das regras de negócio, deixando informações como limite de carga e percentual de ociosidade concentradas em uma estrutura própria. Isso facilita a manutenção, porque caso algum desses valores precise ser alterado, não é necessário mexer em várias partes do código. As regras de ADS utilizam esses dados para verificar se a carga está abaixo do recomendado, dentro do limite ou acima da capacidade máxima. Dessa forma, também fica mais fácil adicionar novas regras e novos indicadores futuramente, deixando o sistema mais organizado, escalável e simples de manter.
